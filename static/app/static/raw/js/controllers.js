...
angular.module('raw.controllers', [])
  ...
  .controller('RawCtrl', function ($scope, dataService, $http, $timeout, $sce, $routeParams) {
    ...
    function getRest(){
      var post_data = {udi:$routeParams.udi,responseType:'arraybuffer'}
      $http({
            url: "/rest/dataset/show",
            method: "POST",
            headers: {'Content-Type': 'application/json; charset=utf-8'},
            data: post_data
        })
      // $http.post("/rest/dataset/show",data)
      .then(function(response) {
        //alert (response.data);
        var bstr = JSON.stringify(response.data);
        try {
          var json = JSON.parse(bstr);
          selectArray(json['data']);
          //select data to show
          $scope.antani(json['data'])
      ...
      function(response){
        ...
        $scope.error = "您提供的文件地址有错误，请确认提供了正确的地址";
      }
    )
    ...
    $scope.$watch('url', function (url) {
      //alert(url);
      //url = $routeParams.http;

      $scope.detail = false;
      if ($routeParams.detail>0){
        $scope.detail = true;
      }
      //alert($routeParams.detail)
      // if(!url || !url.length) {
      //   return;
      // }
    ...
    // $scope.samples = [
    //   { title : 'Biggest cities per continent', type : 'Distributions', url : '/static/raw/data/cities.csv'},
    //   { title : 'Countries GDP', type : 'Other', url : '/static/raw/data/countriesGDP.csv'},
    //   { title : 'Cars', type : 'Multivariate', url : '/static/raw/data/multivariate.csv' },
    //   { title : 'Movies', type : 'Dispersions', url : '/static/raw/data/dispersions.csv' },
    //   { title : 'Music industry', type: 'Time Series', url : '/static/raw/data/music.csv' },
    //   { title : 'Lineup', type : 'Time chunks', url : '/static/raw/data/lineup.tsv' },
    //   { title : 'Orchestras', type : 'Hierarchies (weighted)', url : '/static/raw/data/orchestra.csv' },
    //   { title : 'Animal kingdom', type: 'Hierarchies', url : '/static/raw/data/animals.tsv' },
    //   { title : 'Titanic\'s passengers', type : 'Multi categorical', url : '/static/raw/data/titanic.tsv' },
    //   { title : 'Most frequent letters', type: 'Matrix (narrow)', url:'/static/raw/data/letters.tsv'}
    // ]
...
//    $scope.$watch('sample', function (sample){
    ...
    $(document.getElementById("raw-data")).on('dragenter', function(e){
  ...
  //  $scope.loading = true;
    ...
    $scope.importMode = 'url';// location of mode selection

    //alert('233');
    //$scope.$watch('url')($routeParams.http);

    //$scope.selectSample($scope.samples[0]);
