...
angular.module('raw.directives', [])
	    ...
	    return {
	      ...
	      link: function postLink(scope, element, attrs) {
	        ...
	        function fitWidth(chart, old){
	        	...
	        	scope.option.value = $('.raw-chart').width();
	        }
	...
	.directive('colors', function ($rootScope) {
	    return {
	      restrict: 'A',
	      templateUrl : '/static/raw/templates/colors.html',
	      link: function postLink(scope, element, attrs) {
	      ...
	      function over(e,ui){
		    ...
		    function onRemove(e,ui) {
				...
				function message(){
					...
					scope.messageText = hasInvalidType
						? "此处只能使用 " + scope.types.map(function (d){ return d.name.toLowerCase(); }).join(" 或者 ")
						: "拖拽 " + scope.types.map(function (d){ return d.name.toLowerCase(); }).join("、") + " 到这里";
...
.directive('downloader', function () {
    return {
      ...
      //             '</div>',
			template :  '<div class="row">' +
                    '<form class="form-search col-lg-12">' +
                      '<button bs-select class="btn btn-default" placeholder="选择保存格式" ng-model="mode" bs-options="m.label for m in modes">' +
      ...
      scope.modes = [
    		...
    		// { label : 'Data model (json)', download : downloadData }
				{ label : '矢量图形 (svg)', download : downloadSvg },
    		{ label : '像素图片 (png)', download : downloadPng },
